...
function buildNewsletterText(name: string): string {
  ...
  return `🎁 ${firstName}, дарю вам месяц PRO

Он уже на вашем аккаунте в панели «Комментарии в ПОСТ» – 30 дней, включать ничего не нужно.
...
📊 **Аналитика канала**
Просмотры, комментарии и реакции по дням, топ постов за период до 90 дней. Видно, какой пост зашёл, а какой прошёл мимо.

📁 **Все каналы сразу**
На бесплатном тарифе панель ведёт один канал. С PRO подключаются все, у каждого своя статистика и свои настройки.

🖼 **Фото и стикеры в комментариях**
Подписчики отвечают картинкой или стикером – обсуждение под постом оживает.

─────

Откройте панель кнопкой ниже и загляните в аналитику: там уже накопились данные по вашим постам.`;
}
